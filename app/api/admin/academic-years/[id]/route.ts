...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await db.from("academic_years").update(patch).eq("id", id).select("id,faculty_id,name,is_active").single();
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params;
    const db = createServiceClient();
    const { count } = await db.from("students").select("id", { count: "exact", head: true }).eq("academic_year_id", id);
    if ((count ?? 0) > 0) {
      return NextResponse.json({ error: "لا يمكن الحذف — يوجد طلبة مرتبطين بالسنة." }, { status: 409 });
    }
